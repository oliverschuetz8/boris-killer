...
- Inverse pin scaling via `--pin-eff` CSS variable: `effectivePinSize = max(8px, 24/scale)`, label pill size and font-size all derive from it (font multiplier 0.45, pill min-width multiplier 0.7 — matches the in-app PinBadge math)
...
- **Pin styling alignment (11 May 2026)**: pins on the standalone HTML export used to be a brand-coloured filled circle with the label rendered inside, so they looked different from the in-app red Lucide MapPin everywhere else (Drawings tab, Evidence tab, customer portal). Now the export renders pins identically to the app — inline Lucide MapPin SVG in `#dc2626` red, label in a white pill with slate border + slate text sitting just above the pin's bell, anchor changed from centre-on-coord (`translate(-50%, -50%)`) to tip-on-coord (`translate(-50%, -100%)`) so the pin's tip lands exactly where it was placed. Single file: `lib/html/drawings-export.ts` (`.pin` CSS block + the `l.pins.map(...)` template). Everything else in the export stays untouched — header bottom-border accent, building section accent strip, side-panel pin badge, focus outline, zoom/pan/panel/lightbox behaviour all unchanged
