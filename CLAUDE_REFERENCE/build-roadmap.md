...
- Schedule EventPanel header polish (10 May 2026): on `/schedule` the slide-over panel had its status pill (jobs) and type colour dot (events) in the top-right corner where the shadcn `<Sheet>` close X is rendered → visible collision. Status pill moved into the existing pill row below the title alongside Type / Priority (matches job detail page); colour dot removed from EventBody (icon next to type label already conveys it); both headers given `pr-10` to keep long titles clear of the X. Single file: `app/(dashboard)/schedule/event-panel.tsx`
- Worker view hardening (11 May 2026): three-unit pass closing every place a worker could see admin-only / money information. (A) Money leaks closed — both `JobCostSummary` instances on the job detail page (Overview sidebar + Materials tab) and the "Edit job" button now wrapped in `isAdminOrManager &&`; `/jobs/[id]/edit` page redirects workers. (B) Admin pages locked down with a shared `lib/auth/require-role.ts` helper (`requireAdminOrManager()`) — applied to `/settings`, `/customers`, `/customers/[id]`, `/customers/[id]/edit`, plus a tiny server-wrapper refactor for the two client-component pages (`/customers/new`, `/jobs/new`) so workers can't bypass via URL. Settings sub-pages were already gated. (C) Worker top-nav user menu cleaned up — workers see "Profile" (→ `/profile`) instead of dead-end "Settings". Skipped: photo caption (UI never surfaces it); worker-assignment role swap (not surfaced); old-URL redirect for `/settings/team` (no external links)
- Team page promoted to top-level route (11 May 2026): moved from `/settings/team` to `/team` because every signal in the codebase (file location, back-link to `/settings`, small `text-xl` header) made every chat — including the May 2026 brand consistency pass — treat it as a settings sub-page. `git mv` preserves history; back-link removed; `<h1>` bumped to `text-3xl` to match Customers / Jobs / Invoices / Leads; wrapper padding `px-6 py-8` → `px-8 py-8` per admin-page standard; nav links updated in `top-nav.tsx` and dashboard "Total Tradies" stat card. Failure #14 (Team/Settings nav-highlight clash) is now structurally impossible — `/team` and `/settings` share no prefix — but the rule kept for any future case where two nav items share a prefix. New "Team Page Promoted to Top-Level Route" section added to build-status.md so future chats see it before doing any settings-wide pass
- Admin Edit Parity across job detail page (10 May 2026): closed the gap where admin could only add and delete but not edit. Five units, zero DB changes. (1) Structure tab — building / level / room renames via inline pencil-edit pattern matching the existing prefix pencil. (2) Setup tab — Evidence Fields edit by reusing the Add Field form pre-filled, with type / options / required / default editable. (3) Setup tab — Job Materials Setup edit by reusing the Add Item form pre-filled, with seal_id / manufacturer / system / part-or-product-or-manual swap. (4) Materials tab (admin) — Materials Log edit, full edit for part/manual entries; locked-name read-only with quantity/notes editable for product/legacy entries (with explanatory note to delete-and-re-add for swap). (5) Evidence tab + Drawings tab — full penetration edit modal (new shared `penetration-edit-modal.tsx`) launched from a pencil on each PenetrationCard and from an Edit button in the Drawings-tab pin detail panel. Modal covers subcategory swap (with template-field reload), per-field edits for both template + custom fields, pin label, and Building → Level → Room cascading reassignment (also handles previously Unassigned entries). Pin position itself stays editable in the existing Drawings-tab Move flow; modal includes a hint. New service functions: `updateBuildingName` / `updateLevelName` / `updateRoomName` in building-structure, `updateJobMaterialDefault` in job-material-defaults, `updateRoomMaterial` in room-materials, unified `updatePenetration` in penetrations. `evidence_category_id` plumbed from job-detail-view to both Evidence and Drawings tabs. All edits propagate automatically to customer portal + every export format (single source of truth in Postgres). Skipped: worker-assignment role swap and penetration photo caption (both unused in UI)
